<template>
    <div class="bg-custom">
      ...
      <b-modal v-model="showModal" title="Form Data Layanan" hide-footer>
        <b-form @submit.prevent="saveItem">
          <b-form-group label="Nama Layanan" label-for="namaLayanan">
            <b-form-input
              id="nama_layanan"
  ...
  export default {
    data() {
      return {
        ...
        fields: [
          ...
          { key: 'nama_layanan', label: 'Nama Layanan', sortable: true },
    ...
    methods: {
      async fetchItems() {
        try {
          const response = await axios.get('https://sigapkembiritan.or.id/api/layanans', {
            headers: this.getAuthHeader()
          });
          console.log("Data dari API:", response.data);
          // Memastikan items diambil dari response.data.items
      ...
      async saveItem() {
        try {
          if (this.formData.hashid) {
            ...
            await axios.post(`https://sigapkembiritan.or.id/api/layanans/${this.formData.hashid}`, this.formData, {
              headers: this.getAuthHeader()
            });
          } else {
            // Add new item
            await axios.post('https://sigapkembiritan.or.id/api/layanans', this.formData, {
              headers: this.getAuthHeader()
            });
          }
      ...
      async deleteItem(item) {
        try {
          await axios.delete(`https://sigapkembiritan.or.id/api/layanans/${item.hashid}`, {
            headers: this.getAuthHeader()
          });
          await this.fetchItems(); // Refresh data
        } catch (error) {
